...
def connect_to_database(path: str) -> DuckDBPyConnection:
    logging.info("Connecting to databse at {}")
...
def collect_query_path(parent_dir: str) -> List[str]:
    sql_files = []
...
def setup_database(database_path: str, ddl_query_parent_dir: str) -> None:
    query_paths = collect_query_path(ddl_query_parent_dir)
...
def destroy_databse(database_path: str) -> None:
...
def main():
    ...
    if args.create:
        ...
        destroy_databse(database_path=args.database_path)
